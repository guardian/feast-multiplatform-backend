...
tasks.register("zipXCFramework", Zip::class) {
    ...
    val xcframeworkPath = layout.buildDirectory.dir("XCFrameworks/release/${APIConfig.SPM_FRAMEWORK_NAME}.xcframework")
    from(xcframeworkPath) {
        into("${APIConfig.SPM_FRAMEWORK_NAME}.xcframework")
    }
    archiveFileName.set("${APIConfig.SPM_FRAMEWORK_NAME}.xcframework.zip")
...
tasks.register("publishXCFrameworkToGitHub") {
    ...
    val zipFileProvider = layout.buildDirectory.file("distributions/${APIConfig.SPM_FRAMEWORK_NAME}.xcframework.zip")
    ...
    doLast {
        ...
        let package = Package(
            name: "${APIConfig.SPM_FRAMEWORK_NAME}",
            platforms: [
                .iOS(.v${iosVersion}),
            ],
            products: [
                .library(name: "${APIConfig.SPM_FRAMEWORK_NAME}", targets: ["${APIConfig.SPM_FRAMEWORK_NAME}"])
            ],
            targets: [
                .binaryTarget(
                    name: "${APIConfig.SPM_FRAMEWORK_NAME}",
                    url: "https://github.com/${APIConfig.GITHUB_REPO}/releases/download/$version/${APIConfig.SPM_FRAMEWORK_NAME}.xcframework.zip",
                    checksum:"$checksum")
            ]
        )
        """.trimIndent()
